...
\documentclass[twoside]{cup}  % custom China University of Petroleum project/dissertation/thesis 
...
%% Note that packges may already be loaded from the cup (and memoir) classes.
...
\title{Deep Learning\\Approaches to the\\CBM System}  % use \\ here otherwise you get a justified title
                                                    % note capitalization of the title (only common 
                                                    % words in lower case)
\tagline{New application combined with deep learning methods}                     % tag line
\author{YuHui Song}                           % your name
\authorID{2021010730}                           % your University Identifier
\supervisor{Prof. YanFei Chen}                             % your supervisor(s) name - no . in Dr
\cosupervisor{Prof. YanFei Chen}                               % your cosupervisor(s) name - no . in Dr ** OPTIONAL ** 
                                                    % simply comment out the above line if absent
\department{Department of Oil and Gas Storage and Transportation}                  % your department (e.g. Artifical Intelligence)
\faculty{College of Mechanical and Transportation Engineering}                      % your faculty (e.g. ICT)
\degree{Bachelor}                      % the degree you are reading
                                                    % note the \ after the dot, so not to consider it a fullstop
\doctype{dissertation}                              % the type of document (fyp, dissertation, thesis)
\degreedate{October, 2023}                        % when did you submit -- officially after your corrections !
...
{\backmatter
    ...
    \bibliographystyle{cup-plainnat} %% specific plainnat does not show url for articles
